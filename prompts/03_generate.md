...
**规则要点：**
1. 序号 = 最后一个面试官提问index + 反问QA的index（如面试官最后Q12，反问第一道Q13）。
2. 颜色规则与面试官QA一致（【】标签#1F3864加粗、岗位有效信息正文#00796B、todo #5D4037加粗、信号推断#a2971f），差异点：interviewer_answer 原文不加色
3. candidate_retro_qa 为空时输出：`> （本次面试未检测到候选人反问环节，建议下轮面试准备2-3个有质量的反问问题）`
...
1. 「问题一」标签：#1F3864 加粗；title 原文不加色；description：#0b90cf
2. problems 为空时写：`（本场面试表现较好，暂未发现明显重大问题）`

#### 2.2 怎么改进（表格形式，不加颜色）

```
| todo（含工具、方法论） | 类型 | 参考资源 |
|---|---|---|
| {improvement_plan[0].todo} | {improvement_plan[0].type} | {related_links处理} |
```
- 类型列原样输出"学习项"/"准备项"
- 参考资源：空数组输出 `-`，有链接用 `<br>` 分隔
